...
class PlotMySpec():
    ...
    def __init__(self, paths, conf):
        ...
        if ("crop" in self._conf and self._conf["crop"]):
            self._crop = np.array(
                self._conf["crop"].split(',')).astype(np.float64)
        ...
        for path in paths:
            ...
            if not ('CRPIX1' in spectrum_data["header"]):
                logging.info(
                    '\U0001F5A5 \U0000274C Unable to process %s' % (head_tail[1]))
                continue
            ...
            self._lambda1 = spectrum_data["header"]['CRVAL1'] - \
                lambdaStep * xRef
            self._lambda2 = spectrum_data["header"]['CRVAL1'] + \
                lambdaStep * (xlength - xRef)
            ...
            wavelength = np.arange(
                self._lambda1, self._lambda2, lambdaStep) * u.AA
            # Spectrum construction
            spectrum_data["spec1d"] = Spectrum1D(
                spectral_axis=wavelength, flux=flux)

            spectrum_data["header"]["DATE-OBS"] = spectrum_data["header"]['DATE-OBS'].split('.')[
                0]

            self._spectums_collection[spectrum_data["header"]
                                      ["DATE-OBS"]+str(i)] = spectrum_data
            i += 1

    def parsePattern(self, spec, pattern):
        for header_key in spec['header']:
            pattern = pattern.replace(
                '%%'+header_key+'%%', str(spec['header'][header_key]))
        return pattern
    ...
    def plotSpec(self):
        for spec in self._spectums_collection.values():
            ...
            ax.plot(spec["spec1d"].spectral_axis, spec["spec1d"].flux, label=spec["header"]
                    ['OBJNAME'], alpha=1, color="black", lw=self._conf['line_width'])
            plt.tight_layout(pad=1, w_pad=0, h_pad=0)

            dpi = self._conf['dpi'] if 'dpi' in self._conf else 150
            plt.savefig(pngFilename, dpi=dpi)
            logging.info('\U0001F4C8 Plot %s > save as %s' %
                         (spec["basename"], pngFilename))
            plt.show()
    ...
    def plotSpecGroupMode(self):
        ...
        c = [x.strip() for x in self._conf["compare_mode_color_cycle"].split(
            ',')] if "compare_mode_color_cycle" in self._conf and self._conf["compare_mode_color_cycle"] else None
        if (c and len(c) > 1):
        ...
        for key, spec in sorted(self._spectums_collection.items()):
            ...
            ax.plot(spec["spec1d"].spectral_axis+shift * u.AA, spec["spec1d"].flux,
                    label=label, color=c, alpha=1, lw=self._conf['line_width'])

        if (not "compare_mode_no_label" in self._conf or self._conf["compare_mode_no_label"] != 1):
            if (c and self._offset):
                count = 0
                for key, spec in sorted(self._spectums_collection.items()):
                    label = self.parsePattern(
                        spec, self._conf['label_pattern'])
                    if (self._crop[1]):
                        ax.text(
                            self._crop[1] - 12, 1.07+self._offset[0]*count, label, size='medium')
                    else:
                        ax.text(self._lambda2 - 12, 1.07 +
                                self._offset[0]*count, label, size='medium')
                    count += 1
            else:
                plt.legend()
    ...
    def initPlot(self, spec, withlines=False):
        ...
        fig, ax = plt.subplots(
            figsize=(self._conf["fig_size_x"], self._conf["fig_size_y"]))

        self._spectrum_title = ''
        obj = self._conf['object_name'] if (
            self._conf['object_name']) else spec['header']['OBJNAME']
        split_oname = obj.split(' ')
        for w in split_oname:
            self._spectrum_title += r"$\bf{%s}$ " % (w)
        self._spectrum_title += self.parsePattern(
            spec, self._conf["title_pattern"])
        self._spectrum_subtitle = self.parsePattern(
            spec, self._conf["subtitle_pattern"])

        # Add Graph title
        mff = 'dejavuserif'
        if ('math_font_family' in self._conf):
            mff = self._conf['math_font_family']
        plt.suptitle(self._spectrum_title+'\n'+self._spectrum_subtitle,
                     fontsize=self._conf["title_font_size"], fontweight=0, color='black', math_fontfamily=mff, fontname=self._conf["font_family"])

        # Add X axis label
        ax.set_xlabel(self._conf['x_label'], fontdict=None, labelpad=None,
                      fontname=self._conf["font_family"], size=self._conf["font_size"])

        # Add Y axis label
        ax.set_ylabel(self._conf['y_label'], fontdict=None, labelpad=None,
                      fontname=self._conf["font_family"], size=self._conf["font_size"])

        # Add grid
        if not (self._conf['no_grid']):
            ax.grid(color='grey', alpha=0.4, linestyle='--',
                    linewidth=0.6, axis='both')
            ...
            for line in self._conf['lines']:
                ...
                if line:
                    ...
                    for w in split_oname:
                        n += r"$\bf{%s}$ " % (w)
                    plt.axvline(x=float(
                        lam), color=self._conf['lines_color'], linestyle='--', linewidth=0.7, alpha=0.6)
                    if (self._conf['lines_display_name']):
                        ax.text(float(lam)+float(offset_x), float(offset_y), n+'-'+lam+' Å',
                                rotation=90, va='bottom', color=self._conf['lines_color'], size='7')

        # Crop spectrum
        ax.set_xlim(self._crop[:2]) if (len(self._crop) >= 2) else ax.set_xlim(
            [self._lambda1, self._lambda2])
        if (len(self._crop) == 4):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--config", type=str,
                        default=default_conf_filename, help="Custom config file name")
    ...
    if confpath:
        logging.info(
            '\U00002728 Load configuration file \U0001F527  %s' % (confpath))
